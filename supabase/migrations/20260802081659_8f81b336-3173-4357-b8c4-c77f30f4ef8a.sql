...
DECLARE
  ...
  o record; msg text; items text := ''; it record; req_id bigint;
BEGIN
  ...
  INSERT INTO public.whatsapp_log(order_id, url, chat_id, reason, request_id)
    VALUES (_order_id, v_masked, v_chat, 'sent', req_id);
...
CREATE OR REPLACE FUNCTION public.whatsapp_test_send()
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
REVOKE ALL ON FUNCTION public.whatsapp_test_send() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.whatsapp_test_send() TO anon, authenticated, service_role;